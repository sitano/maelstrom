    ...
    (case (:type body)
      ...
      "decide"  (if (not_decided? i)
                  (let [[con res] (handle con message)
                    ...
                    (if (= (:type res) "error")
                      [this res]
                      [(append-log this [i, (get-in res [:value])]) res]))
                  [this {:type "error", :code 22, :text "already decided", :i i}])
      ; not supported yet.
...
;       a
;       [#maelstrom.service.MSDPaxos{:log ([1 1]), :con #maelstrom.service.Consensus{:m ()}, :i 1} {:type "decide_ok", :value 1}]
;
;       (def a (handle (first a) {:body {:type "decide"}}))
;       a
...
;       (def a (handle (first a) {:body {:type "decide"}}))
;       a
;       [#maelstrom.service.MSDPaxos{:log ([3 3]), :con #maelstrom.service.Consensus{:m ()}, :i 3} {:type "decide_ok", :value 3}]
;       (def a (handle (first a) {:body {:type "decide"}}))
;       a
