...
# $ ./lein run -- test -w lin-kv --bin demo/ruby/lin_kv_sdp.rb --time-limit 10 --node-count 3 --rate 1 --concurrency 2n --key-count 1
...
class LinKVNode
  def initialize
    ...
    # TODO: can we run with delays?
    # TODO: fix it
    @node.on "cas" do |msg|
      STDERR.puts("- cas req key(#{msg[:body][:key]}) = #{msg[:body][:to]} if #{msg[:body][:from]}")
      ok, prev = consensus! msg 
      if ok
        @node.reply! msg, { type: "cas_ok" }
      elsif prev.nil?
        @node.reply! msg, { type: "error", code: 20, text: "key does not exist" }
      else
        @node.reply! msg, { type: "error", code: 22, text: "current value #{prev} is not #{msg[:body][:from]}" }
      end
    end
  ...
  def apply_learned!(promise, pid, msg)
    ...
    when "cas"
      prev = @storage[body[:key]]
      if prev == body[:from]
        @storage[body[:key]] = body[:to]
        STDERR.puts("+ cas_ok key(#{body[:key]}) = #{body[:to]}")
        promise.deliver! [true, prev] if promise
      else 
        STDERR.puts("- cas_err key(#{body[:key]}) = #{body[:to]}")
        promise.deliver! [false, prev] if promise
      end
    when "read"
